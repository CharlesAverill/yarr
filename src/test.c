...
class Object
{
  public:
    Object()
        : color(255, 255, 255), reflectivity(1), hardness(1), diffuse_factor(1), specular_factor(1),
          roughness(0)
    {
    }
                       ...
                       Vector &object_normal) const
    {
        ...
        if (b > threshold) {
            ...
            outgoing_ray_direction =
                !(incoming_ray_direction + !normal * (!normal % -incoming_ray_direction) * 2);
  ...
  public:
                       ...
                       Vector &object_normal) const
    {
        ...
        const float u_factor =
            (-(ox - pox) * (ry * vz - rz * vy) + (oy - poy) * (rx * vz - rz * vx) -
             (oz - poz) * (rx * vy - ry * vx)) /
            (rx * uy * vz - rx * uz * vy - ry * ux * vz + ry * uz * vx + rz * ux * vy -
             rz * uy * vx);
        const float v_factor =
            ((ox - pox) * (ry * uz - rz * uy) - (oy - poy) * (rx * uz - rz * ux) +
             (oz - poz) * (rx * uy - ry * ux)) /
            (rx * uy * vz - rx * uz * vy - ry * ux * vz + ry * uz * vx + rz * ux * vy -
             rz * uy * vx);
        const float ray_factor =
            (-(ox - pox) * (uy * vz - uz * vy) + (oy - poy) * (ux * vz - uz * vx) -
             (oz - poz) * (ux * vy - uy * vx)) /
            (rx * uy * vz - rx * uz * vy - ry * ux * vz + ry * uz * vx + rz * ux * vy -
             rz * uy * vx);

        if (u_factor < 0 or u_factor > 1 or v_factor < 0 or v_factor > 1 or
            u_factor + v_factor > 1 or ray_factor < 0)
            return false;
...
Vector get_ground_color(const Vector &ray_origin, const Vector &ray_direction, Vector &ray_hit_at)
{
    //return {255, 0, 0};
...
int main()
{
    ...
    /// Bottom half
    scene_objects.push_back(new Triangle{Vector{0, 0, 0}, Vector{-1, 1, 0}, Vector{0, 1, 1}});
    scene_objects.back()->set_diffuse_factor(0);
    scene_objects.push_back(new Triangle{Vector{0, 0, 0}, Vector{0, 1, -1}, Vector{-1, 1, 0}});
    scene_objects.back()->set_diffuse_factor(0);
    scene_objects.push_back(new Triangle{Vector{0, 0, 0}, Vector{1, 1, 0}, Vector{0, 1, -1}});
    scene_objects.back()->set_diffuse_factor(0);
    scene_objects.push_back(new Triangle{Vector{0, 0, 0}, Vector{0, 1, 1}, Vector{1, 1, 0}});
    scene_objects.back()->set_diffuse_factor(0);
    /// Top half
    scene_objects.push_back(new Triangle{Vector{0, 2, 0}, Vector{0, 1, 1}, Vector{-1, 1, 0}});
    scene_objects.back()->set_diffuse_factor(0);
    scene_objects.push_back(new Triangle{Vector{0, 2, 0}, Vector{1, 1, 0}, Vector{0, 1, 1}});
    scene_objects.back()->set_diffuse_factor(0);
    scene_objects.push_back(new Triangle{Vector{0, 2, 0}, Vector{0, 1, -1}, Vector{1, 1, 0}});
    scene_objects.back()->set_diffuse_factor(0);
    scene_objects.push_back(new Triangle{Vector{0, 2, 0}, Vector{-1, 1, 0}, Vector{0, 1, -1}});
    ...
    for (int y = 256; y >= -255; --y) {
        ...
        for (int x = -255; x <= 256; ++x) {
            ...
            Vector ray_direction = !Vector{X * (x - 0.5f) + Y * (y - 0.5f) + Z};
            ...
            for (int bounce = 0; bounce <= 100; ++bounce) {
                ...
                if (an_object_was_hit and closest_object_ptr) {
                    ...
                    if (ray_direction.y < 0) {
                        ...
                        normal = {0, 1, 0};
                        ray_bounced_direction =
                            !(ray_direction + !normal * (-ray_direction % normal) * 2);
                        ray_origin = ray_hit_at;
                        ray_direction = ray_bounced_direction;
                ...
                if (not sky_was_hit) {
                    ...
                    const float specular_factor{
                        std::max(0.f, !(light_at - ray_hit_at) % ray_direction)};
                    color = color * ambient_light + color * diffuse_light * diffuse_factor_at_hit +
                            light_color * std::pow(specular_factor, hardness_at_hit) *
                                specular_factor_at_hit;
                }
            ...
            outfile << static_cast<unsigned char>(std::max(0.f, std::min(255.f, final_color.x)))
